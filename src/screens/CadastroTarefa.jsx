...
function CadastroTarefa({ navigation }) {
    ...
    const criarTarefa = async () => {
        let statement = await db.prepareAsync(`
            ...
            `);
        try {
            let result = await statement.executeAsync({ $descricao: text, $situacao: 'P' });
            if (result.lastInsertRowId !== null) {
                console.log('tarefa salvou');
    ...
    return (
        <View style={{ flex: 1, gap: 10, padding: 10, justifyContent: 'space-between' }}>
            <TextInput
                ...
                mode="outlined"
            />
            <Button icon="content-save" mode="contained" onPress={() => criarTarefa()} compact={true}>
                Salvar
            </Button>
        </View>
    );
